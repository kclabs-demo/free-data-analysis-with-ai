...
Example analyses built with this setup, all querying BigQuery public datasets via Python + `google-cloud-bigquery`.

### [Language vs Reputation](./analysis/language-reputation/)

Explores the relationship between programming language choice and developer reputation on Stack Overflow, using reputation as a proxy for salary/expertise.

- **Dataset**: `bigquery-public-data.stackoverflow` (18.7M users, 23M questions)
- **Method**: Single-tag questions joined with users (reputation >= 100, min 100 users per tag)
- **Top finding**: "open-source" tag users have the highest avg reputation (28,118), followed by "filemaker" (27,098) and "scons" (24,492)
- **Quality**: 8/8 checks passed (row counts, nulls, filters, integrity)
- **Files**: SQL exploration/creation, Python analysis scripts, JSON/CSV results

### [Stack Overflow Question Count](./analysis/stackoverflow-question-count/)

Simple verification query confirming dataset scale: **23,020,127 total questions** in the Stack Overflow public dataset.
...
├── .internal/                      # Kyle's private files (gitignored)
│   ├── OWNER_CONFIG.md
│   ├── RECORDING_NOTES.md
│   └── OPENCODE_PROMPTS.md
├── example_workflow/
...
├── images/
│   ├── diagram-connection.excalidraw
│   └── diagram-connection.png
├── instructions/
│   └── setup_guide.md              # Condensed setup guide
└── analysis/
    ├── language-reputation/         # Language vs reputation analysis
    │   ├── sql/
    │   │   ├── exploration.sql
    │   │   └── create_table.sql
    │   ├── python/
